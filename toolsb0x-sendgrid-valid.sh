...
EOF
echo -e "=========================[ ${GINGIN}Sendgrid User Validator${WHITE} ]========================="

echo -e "${GREEN}Sendgrid User Validator by Toolsb0x.com${WHITE}"
...
check(){
    cr=$(curl -s 'http://api.toolsb0x.com/api/sendgrid-valid.php?list='$email'&token='$YOURTOKEN'&wallet='$YOURWALLET'')
     if [[ $cr =~ '"status":"live"' ]]; then
        echo -e "[$2/$total] ${GREEN}SENDGRID LIVE => $email${WHITE} [ Toolsb0x.com - [$date]]"
        echo $email "[ Toolsb0x.com - [$date]]" >> Sendgrid-live.txt
    elif [[ $cr =~ '"status":"die"' ]]; then
        echo -e "[$2/$total] ${RED}SENDGRID DIE => $email${WHITE}"
        echo $email >> Sendgrid-dead.txt
    elif [[ $cr =~ '"status":"unknown"' ]]; then
        echo -e "[$2/$total] ${KUNING}SENDGRID UNKNOWN => $email${WHITE}"
        echo $email >> Sendgrid-uncheck.txt
	elif [[ $cr =~ '"status":"connection"' ]]; then
        echo -e "[$2/$total] ${KUNING}CONNECTIONFAILURE => $email${WHITE}"
        echo $email >> Sendgrid-connectionfailure.txt
    else [[ $cr =~ '"status":"insufficient"' ]]; 
        echo -e "[$2/$total] ${GONGON}insufision balance => $email${WHITE}"
        echo $email >> Sendgrid-insufisionbalance.txt
    fi
}
